...
class Scene(private val window: GameWindow)  {
    ...
    init {
        ...
        cent = ModelLoader.loadModelCoin("assets/models/50cent.obj",Math.toRadians(90.0f),Math.toRadians(180.0f),Math.toRadians(90.0f)) ?: throw IllegalArgumentException("Could not load the model")
       ...
       material = Material(diff,emit,spec,60f, Vector2f(128f,128f))
    ...
    fun update(dt: Float, t: Float) {
        ...
        var cycle_pos = person.getPosition()
        if((cycle_pos[0] >= cent_pos[0]-0.5f && cycle_pos[0] <= cent_pos[0]+0.5f) && (cycle_pos[2] >= cent_pos[2]-0.5f && cycle_pos[2] <= cent_pos[2]+0.5f)) {
