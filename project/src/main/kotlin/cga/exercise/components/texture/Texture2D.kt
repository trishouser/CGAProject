...
class Texture2D(imageData: ByteBuffer, width: Int, height: Int, genMipMaps: Boolean): ITexture{
    ...
    override fun processTexture(imageData: ByteBuffer, width: Int, height: Int, genMipMaps: Boolean) {
        ...
        glTexImage2D(GL_TEXTURE_2D,0,GL_RGBA8,width,height,0,GL_RGBA, GL11.GL_UNSIGNED_BYTE,imageData)
        if(genMipMaps) {
    ...
    override fun setTexParams(wrapS: Int, wrapT: Int, minFilter: Int, magFilter: Int) {
        bind(0)
        glTexParameteri(GL_TEXTURE_2D, GL11.GL_TEXTURE_WRAP_S,wrapS)
        glTexParameteri(GL_TEXTURE_2D, GL11.GL_TEXTURE_WRAP_T,wrapT)
        glTexParameteri(GL_TEXTURE_2D, GL11.GL_TEXTURE_MIN_FILTER,minFilter)
        glTexParameteri(GL_TEXTURE_2D, GL11.GL_TEXTURE_MAG_FILTER,magFilter)
